import numpy as np
import gymnasium as gym
from gymnasium import spaces


class DiabetesFollowUpEnv(gym.Env):
    """Simulador mensual de seguimiento para una cohorte de diabetes."""

    metadata = {"render_modes": []}

    def __init__(
        self,
        cohorte_df,
        capacidad_mensual=0.20,
        horizonte=12,
        seed=None
    ):
        super().__init__()

        self.cohorte_base = cohorte_df.reset_index(drop=True).copy()
        self.n_perfiles = len(self.cohorte_base)
        self.capacidad_mensual = float(capacidad_mensual)
        self.horizonte = int(horizonte)

        self.observation_space = spaces.Box(
            low=0.0,
            high=1.0,
            shape=(8,),
            dtype=np.float32
        )

        self.action_space = spaces.Discrete(4)

        self.costo_accion = {
            0: 0,
            1: 1,
            2: 2,
            3: 3
        }

        self._rng = np.random.default_rng(seed)

    def reset(self, seed=None, options=None):
        super().reset(seed=seed)

        if seed is not None:
            self._rng = np.random.default_rng(seed)

        self.cohorte = self.cohorte_base.copy()
        self.cohorte["meses_sin_contacto"] = 0

        self.mes_actual = 1
        self.idx_actual = 0

        self.presupuesto_mensual = int(
            self.n_perfiles * self.capacidad_mensual
        )

        self.recursos_usados_mes = 0
        self.acciones_aceptadas_mes = 0
        self.intentos_exceso_mes = 0

        obs = self._get_obs(self.idx_actual)

        info = {
            "mes": self.mes_actual,
            "n_perfiles": self.n_perfiles,
            "presupuesto_mensual": self.presupuesto_mensual,
            "recursos_usados_mes": self.recursos_usados_mes
        }

        return obs, info

    def _get_obs(self, idx):
        row = self.cohorte.iloc[idx]

        riesgo_map = {
            "bajo": 0.0,
            "medio": 0.5,
            "alto": 1.0
        }

        sexo = float(row["sexo"])

        obs = np.array([
            np.clip(float(row["edad"]) / 100.0, 0.0, 1.0),
            np.clip(sexo, 0.0, 1.0),
            np.clip(
                float(row["anios_con_diabetes"]) / 50.0,
                0.0,
                1.0
            ),
            np.clip(
                float(row["num_complicaciones"]) / 9.0,
                0.0,
                1.0
            ),
            np.clip(
                float(row["num_comorbilidades"]) / 2.0,
                0.0,
                1.0
            ),
            riesgo_map.get(
                row["categoria_riesgo"],
                0.0
            ),
            np.clip(
                float(row["meses_sin_contacto"]) / 12.0,
                0.0,
                1.0
            ),
            np.clip(
                float(self.mes_actual) / self.horizonte,
                0.0,
                1.0
            )
        ], dtype=np.float32)

        return obs

    def step(self, action):
        action = int(action)

        if not self.action_space.contains(action):
            raise ValueError(f"Acción inválida: {action}")

        costo_solicitado = self.costo_accion[action]

        excede_capacidad = (
            self.recursos_usados_mes + costo_solicitado
            > self.presupuesto_mensual
        )

        if excede_capacidad:
            accion_aceptada = False
            costo_real = 0
            self.intentos_exceso_mes += 1
        else:
            accion_aceptada = True
            costo_real = costo_solicitado
            self.recursos_usados_mes += costo_real

            if action > 0:
                self.acciones_aceptadas_mes += 1

        row = self.cohorte.iloc[self.idx_actual]

        riesgo_map = {
            "bajo": 0,
            "medio": 1,
            "alto": 2
        }

        nivel_riesgo = riesgo_map.get(
            row["categoria_riesgo"],
            0
        )

        prob_evento_base = 0.02 + 0.03 * nivel_riesgo

        reduccion_por_accion = {
            0: 0.0,
            1: 0.3,
            2: 0.6,
            3: 0.85
        }

        prob_evento = (
            prob_evento_base
            * (1 - reduccion_por_accion[action])
        )

        evento_adverso = bool(
            self._rng.random() < prob_evento
        )

        reward = 0.0

        if action >= 2 and nivel_riesgo == 2:
            reward += 2.0

        if evento_adverso:
            reward -= 5.0

        if excede_capacidad:
            reward -= 3.0

        if action == 0:
            self.cohorte.loc[
                self.idx_actual,
                "meses_sin_contacto"
            ] += 1
        else:
            self.cohorte.loc[
                self.idx_actual,
                "meses_sin_contacto"
            ] = 0

        self.idx_actual += 1

        terminated = False
        truncated = False

        if self.idx_actual >= self.n_perfiles:
            self.idx_actual = 0
            self.mes_actual += 1

            self.recursos_usados_mes = 0
            self.acciones_aceptadas_mes = 0
            self.intentos_exceso_mes = 0

            if self.mes_actual > self.horizonte:
                terminated = True

        if terminated:
            obs = np.zeros(
                self.observation_space.shape,
                dtype=np.float32
            )
        else:
            obs = self._get_obs(self.idx_actual)

        info = {
            "mes": self.mes_actual,
            "evento_adverso": evento_adverso,
            "excede_capacidad": excede_capacidad,
            "accion_aceptada": accion_aceptada,
            "costo_solicitado": costo_solicitado,
            "costo_real": costo_real,
            "recursos_usados_mes": self.recursos_usados_mes,
            "presupuesto_mensual": self.presupuesto_mensual,
            "acciones_aceptadas_mes": (
                self.acciones_aceptadas_mes
            ),
            "intentos_exceso_mes": (
                self.intentos_exceso_mes
            )
        }

        return obs, float(reward), terminated, truncated, info
